...
public final class GigaToolUtils {
    ...
    public static FewShotExample[] getFewShotExamples(Method method) {
        Assert.notNull(method, "method cannot be null");
        var gigaTool = method.getAnnotation(GigaTool.class);
        var tool = method.getAnnotation(Tool.class);
        var exampleList = method.getAnnotation(FewShotExampleList.class);
        var example = method.getAnnotation(ai.forever.gigachat.tool.annotation.FewShotExample.class);
        if (gigaTool == null && tool == null) {
            return new FewShotExample[0];
        }
        Stream<ai.forever.gigachat.tool.annotation.FewShotExample> shotExampleStream = Stream.empty();
        ...
        return shotExampleStream
                .map(fewShotExample -> FewShotExample.builder()
                        .request(fewShotExample.request())
                        .paramsSchema(fewShotExample.params())
                        .build())
                .toArray(FewShotExample[]::new);
    }
